...
from .config import SENSOR_ID_INDEX

#센서 데이터 리스트를 사용자 ID별로 분류합니다.
def classify_data_by_id(sensor_data_list):
    data_by_id = defaultdict(list)
    for data in sensor_data_list:
        data_by_id[data[SENSOR_ID_INDEX]].append(data)
    return data_by_id


#데이터 포인트의 카테고리와 날짜를 결정합니다.
def classify_category(timestamp, sunrise_today_time, sunset_today_time, current_date):
